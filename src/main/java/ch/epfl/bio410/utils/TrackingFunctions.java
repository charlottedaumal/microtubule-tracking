...
public class TrackingFunctions {
     ...
     * @param allSpots all the other spots in the image
     ...
     */
    public static DirectionVector findDirection(Spot spot, Spots allSpots, int dimension) {
        Spot previousSpot = null;
        int halfDim = dimension / 2;
        int tPrev = spot.t - 1;
        double minDist = Double.MAX_VALUE;

        for (Spot other : allSpots) {
            if (other.t == tPrev) {
